...
RUN apt-get update && apt-get install -y build-essential cmake git nasm \
 ...
 && cd /SDL3_image \
 && git submodule update --init --recursive \
 && cmake -B build -S . -DSDLIMAGE_VENDORED=ON -DAOM_NEON_INTRIN_FLAG=-mfpu=neon $CMAKE_ARGS \
 && cmake --build build --parallel \
 && cmake --install build \
 && cd / \
 && rm -rf /SDL3_image \
 ...
 && cd /SDL2_image \
 && git submodule update --init --recursive \
 && cmake -B build -S . -DSDL2IMAGE_VENDORED=ON -DAOM_NEON_INTRIN_FLAG=-mfpu=neon $CMAKE_ARGS \
 && cmake --build build --parallel \
 && cmake --install build \
 && cd / \
 && rm -rf /SDL2_image \
 ...
 && cd /SDL2_mixer \
 && git submodule update --init --recursive \
 && cmake -B build -S . -DSDL2MIXER_VENDORED=ON $CMAKE_ARGS \
 && cmake --build build --parallel \
 && cmake --install build \
 && cd / \
 && rm -rf /SDL2_mixer \
...
# && cd /SDL3_mixer \
# && git submodule update --init --recursive \
# && cmake -B build -S . -DSDLMIXER_VENDORED=ON $CMAKE_ARGS \
# && cmake --build build --parallel \
# && cmake --install build \
# && cd / \
# && rm -rf /SDL3_mixer \
